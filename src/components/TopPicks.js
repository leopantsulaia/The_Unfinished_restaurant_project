...
const TopPicks = () => {
	return (
		<div>
			...
			<div className="hidden lg:flex max-w-screen-2xl m-auto py-2 px-2">
				<Splide options={{ perPage: 4, gap: "2rem", drag: "free", 
        // arrows: false
         }}>
					{topPicks.map((item) => {
						return (
							<SplideSlide key={item.id}>
								<div className="rounded-3xl relative">
									<div className="absolute w-full h-full bg-black/50 rounded-3xl text-white">
										<p className="px-2 pt-5 font-bold text-2xl ">{item.title}</p>
